...
export default function PortfelTab({ onOpenAssetChart }: PortfelTabProps) {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      // For Bybit REAL mode, use futures balance API (shows sub-account balance correctly)
      const baseUrl = exchange === 'binance' ? '/api/binance/balance' 
        : (mode === 'real' ? '/api/bybit/futures/balance' : '/api/bybit/balance')
      const res = await fetch(`${baseUrl}?mode=${mode}`)
      if (res.ok) {
        const data = await res.json()
        // Futures balance API has different structure
        const totalEquityUsdt = data.totalEquityUsdt || data.totalEquityUsdt || 0
        const coins = data.coins || []
        const subAccountName = data.subAccountName || null
        const totalUnrealisedPnl = data.totalUnrealisedPnl || 0
        const availableBalance = data.availableBalance || 0
        const totalWalletBalance = data.totalWalletBalance || 0
        ...
        setHistory(prev => {
          ...
          const entry = { time: now, value: totalEquityUsdt || 0 }
          const newHist = [...prev, entry]
          return newHist.length > 60 ? newHist.slice(-60) : newHist
        })
      } else { const data = await res.json(); setError(data.error || 'Błąd pobierania salda') }
  ...
  useEffect(() => {
    ...
    if (!bybitOk && binanceOk) setExchange('binance')
    if (bybitOk || binanceOk) void fetchBalance()
  ...
  const handleSellAll = useCallback(async () => {
    ...
    try {
      ...
      else { setSellAllResult({ message: data.error || 'Błąd sprzedaży', soldCount: 0, errorCount: 1 }) }
    } catch { setSellAllResult({ message: 'Błąd połączenia z serwerem', soldCount: 0, errorCount: 1 }) }
      ...
      <div className="max-w-md mx-auto space-y-3">
          ...
          <div className="flex items-center gap-1.5">
            {isConfigured && nonUsdtCoins.length > 0 && (
              <Dialog open={sellAllConfirm} onOpenChange={setSellAllConfirm}>
                ...
                <DialogContent className="max-w-xs"><DialogHeader className="sr-only"><DialogTitle><VisuallyHidden.Root>Potwierdź sprzedaż wszystkich aktywów</VisuallyHidden.Root></DialogTitle></DialogHeader>
                  <div className="text-center py-4 space-y-3">
                    ...
                    <div><p className="text-sm font-bold">Sprzedać wszystko?</p><p className="text-xs text-muted-foreground mt-1">Market sell {nonUsdtCoins.length} aktywów na {exchange === 'bybit' ? 'Bybit' : 'Binance'} ({mode})</p></div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-muted rounded-md p-0.5 text-xs">
            ...
            <button className={`px-2 py-1 rounded-sm font-medium transition-colors ${exchange === 'binance' ? 'bg-yellow-600 text-white' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setExchange('binance')}>BNB</button>
          </div>
          <div className="flex items-center bg-muted rounded-md p-0.5 text-xs">
        ...
        {!isConfigured && <Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="py-6 text-center"><Key className="size-8 mx-auto mb-2 text-muted-foreground" /><p className="text-xs text-muted-foreground">Klucze API dla {exchange === 'bybit' ? 'Bybit' : 'Binance'} ({mode}) nie są skonfigurowane.</p><p className="text-xs text-muted-foreground mt-1">Kliknij ⚙️ w nagłówku, aby dodać klucze.</p></CardContent></Card>}
        {isConfigured && <Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="pt-4 pb-3 px-4"><div className="flex items-center justify-between mb-1"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Wartość portfela</span>{balance?.lastUpdated && <span className="text-[9px] text-muted-foreground">{new Date(balance.lastUpdated).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>}</div><div className="text-2xl font-bold tracking-tight">${balance?.totalEquityUsdt.toFixed(2) || '0.00'}</div>{exchange === 'bybit' && mode === 'real' && balance ? (
  <div className="text-[10px] text-muted-foreground mt-0.5 space-y-0.5">
    <div>
      Wallet: ${(balance.totalWalletBalance ?? 0).toFixed(2)} | Available: ${(balance.availableBalance ?? 0).toFixed(2)}
    </div>
    {(balance.totalUnrealisedPnl ?? 0) !== 0 && (
      <div className={balance.totalUnrealisedPnl! >= 0 ? 'text-emerald-500' : 'text-red-500'}>
        Unrealized PnL: {balance.totalUnrealisedPnl! >= 0 ? '+' : ''}{(balance.totalUnrealisedPnl ?? 0).toFixed(2)}
      </div>
    )}
    {balance.subAccountName && (
      <div className="text-[9px] text-amber-500">
        Sub: {balance.subAccountName}
      </div>
    )}
  </div>
) : usdtCoin ? (
  <div className="text-[10px] text-muted-foreground mt-0.5">
    USDT: {Number(usdtCoin.walletBalance).toFixed(2)} ({Number(usdtCoin.free).toFixed(2)} dostępne)
  </div>
) : null}</CardContent></Card>}
        {isConfigured && history.length > 1 && (<Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="pt-3 pb-2 px-4"><div className="flex items-center justify-between mb-1"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Krzywa kapitału</span>{capitalChange !== 0 && <span className={`text-[10px] font-semibold ${capitalChange >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>{capitalChange >= 0 ? '+' : ''}{capitalChange.toFixed(2)} ({capitalChangePct >= 0 ? '+' : ''}{capitalChangePct.toFixed(2)}%)</span>}</div>
          <ChartContainer config={{ value: { label: 'Kapitał', color: '#10b981' } }} className="w-full h-[100px] mt-1"><AreaChart data={history} margin={{ top: 4, right: 4, bottom: 2, left: 4 }}><defs><linearGradient id="capCurveGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={capitalChange >= 0 ? '#10b981' : '#ef4444'} stopOpacity={0.25} /><stop offset="95%" stopColor={capitalChange >= 0 ? '#10b981' : '#ef4444'} stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" opacity={0.3} /><XAxis dataKey="time" tick={{ fontSize: 8, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} interval="preserveStartEnd" /><YAxis tick={{ fontSize: 8, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} width={40} tickFormatter={(v: number) => `$${v.toFixed(0)}`} domain={['auto', 'auto']} /><ChartTooltip content={<ChartTooltipContent formatter={(value) => [`$${Number(value).toFixed(2)}`, 'Kapitał']} />} /><Area type="monotone" dataKey="value" stroke={capitalChange >= 0 ? '#10b981' : '#ef4444'} strokeWidth={2} fill="url(#capCurveGrad)" dot={history.length <= 10} isAnimationActive={false} /></AreaChart></ChartContainer></CardContent></Card>)}
        {isConfigured && nonUsdtCoins.length > 0 && (<Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="pt-3 pb-2 px-3"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Aktywa ({nonUsdtCoins.length})</span><div className="mt-2 space-y-1">{nonUsdtCoins.map(coin => { const equity = Number(coin.equity); const locked = Number(coin.locked); const pnl = Number(coin.unrealisedPnl); return (<div key={coin.coin} className="flex items-center justify-between bg-neutral-100 dark:bg-neutral-800 rounded-md px-2.5 py-1.5"><div className="flex items-center gap-2"><div className="size-5 rounded-full bg-gradient-to-br from-primary/30 to-primary/60 flex items-center justify-center"><span className="text-[8px] font-bold text-primary">{coin.coin.slice(0, 2)}</span></div><div><div className="text-xs font-semibold">{coin.coin}</div>{locked > 0 && <div className="text-[9px] text-muted-foreground">{locked.toFixed(4)} zablokowane</div>}</div></div><div className="flex items-center gap-2"><div className="text-right"><div className="text-xs font-semibold">{equity.toFixed(4)}</div>{pnl !== 0 && <div className={`text-[9px] ${pnl >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>{pnl >= 0 ? '+' : ''}{pnl.toFixed(4)}</div>}</div>{onOpenAssetChart && <button onClick={() => onOpenAssetChart(cryptoTickerToTvSymbol(coin.coin))} className="shrink-0 px-1.5 py-0.5 text-[7px] font-mono uppercase tracking-wider border border-border rounded hover:bg-accent hover:text-accent-foreground transition-colors text-muted-foreground" title="Pokaż na wykresie TradingView">📈 Wykres</button>}</div></div>) })}</div></CardContent></Card>)}
