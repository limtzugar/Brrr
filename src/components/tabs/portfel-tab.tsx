...
export default function PortfelTab({ onOpenAssetChart }: PortfelTabProps) {
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] bg-neutral-200 dark:bg-neutral-800 rounded-xl p-4">
      <div className="max-w-md mx-auto space-y-3">
              ...
              <Dialog open={sellAllConfirm} onOpenChange={setSellAllConfirm}>
                ...
                <DialogContent className="max-w-xs"><DialogHeader className="sr-only"><DialogTitle><VisuallyHidden.Root>Confirm sell all assets</VisuallyHidden.Root></DialogTitle></DialogHeader>
                  <div className="text-center py-4 space-y-3">
                    ...
                    <div><p className="text-sm font-bold">Sell everything?</p><p className="text-xs text-muted-foreground mt-1">Market sell {nonUsdtCoins.length} assets on {exchange === 'bybit' ? 'Bybit' : 'Binance'} ({mode})</p></div>
        ...
        {!isConfigured && <Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="py-6 text-center"><Key className="size-8 mx-auto mb-2 text-muted-foreground" /><p className="text-xs text-muted-foreground">API keys for {exchange === 'bybit' ? 'Bybit' : 'Binance'} ({mode}) are not configured.</p><p className="text-xs text-muted-foreground mt-1">Click ⚙️ in header to add keys.</p></CardContent></Card>}
        {isConfigured && <Card className="bg-white dark:bg-neutral-900 border shadow-sm"><CardContent className="pt-4 pb-3 px-4"><div className="flex items-center justify-between mb-1"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Portfolio value</span>{balance?.lastUpdated && <span className="text-[9px] text-muted-foreground">{new Date(balance.lastUpdated).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>}</div><div className="text-2xl font-bold tracking-tight">${balance?.totalEquityUsdt.toFixed(2) || '0.00'}</div>{exchange === 'bybit' && mode === 'real' && balance ? (
  ...
  <div className="text-[10px] text-muted-foreground mt-0.5">
    USDT: {Number(usdtCoin.walletBalance).toFixed(2)} ({Number(usdtCoin.free).toFixed(2)} available)
  </div>
