...
function DrawdownHeatMap() {
  ...
  if (heatMapData.length === 0) return <div className="text-xs text-muted-foreground text-center py-4">Loading heatmap data...</div>

  return (<>
    <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr><th className="text-left pb-2 pr-3 font-medium text-muted-foreground">Coin</th>{Array.from({ length: heatMapDays }, (_, i) => (<th key={i} className="text-center pb-2 px-1 font-medium text-muted-foreground">{i === 0 ? 'Today' : `-${i}d`}</th>))}</tr></thead>
    ...
    {heatMapTooltip && <div className="mt-2 text-xs text-muted-foreground bg-muted/50 rounded px-3 py-1.5"><span className="font-medium">{heatMapTooltip.coin}</span> — day {heatMapTooltip.day === 0 ? 'today' : `-${heatMapTooltip.day}d`}: <span className={heatMapTooltip.drawdown < -5 ? 'text-red-500 font-medium' : 'text-amber-500'}>{heatMapTooltip.drawdown.toFixed(2)}%</span></div>}
...
export default function BacktestTab() {
  ...
  const runBacktestFn = async () => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d.error || 'Backtest failed') }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Brain className="size-4" />Konfiguracja Backtestu</CardTitle><CardDescription>Ustaw parametry strategii — wybierz typ i dostosuj</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              ...
              {strategyType === 'dip_buying' && (<div><Label className="text-xs text-muted-foreground">Preset strategies</Label><div className="flex gap-2 mt-1">{presetStrategiessss.map(p => (<Button key={p.name} variant="outline" size="sm" className="text-xs" onClick={() => { setDipThreshold1h(p.dip1h); setDipThreshold24h(p.dip24h); setTakeProfitPct(p.tp); setStopLossPct(p.sl); setDays(p.days) }}>{p.name}</Button>))}</div></div>)}
              <div><Label className="text-xs">Coin</Label><Select value={coinId} onValueChange={setCoinId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{COIN_OPTIONS.map(c => <SelectItem key={c.id} value={c.id}>{c.label}</SelectItem>)}</SelectContent></Select></div>
              <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Period (days)</Label><Input type="number" value={days} onChange={e => setDays(Number(e.target.value))} min={30} max={730} /></div><div><Label className="text-xs">Initial capital ($)</Label><Input type="number" value={initialCapital} onChange={e => setInitialCapital(Number(e.target.value))} min={100} /></div></div>
              ...
              {(strategyType === 'momentum') && <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">MA period</Label><Input type="number" value={maPeriod} onChange={e => setMaPeriod(Number(e.target.value))} min={2} /></div><div><Label className="text-xs">Volume threshold (x)</Label><Input type="number" value={volumeThreshold} onChange={e => setVolumeThreshold(Number(e.target.value))} step={0.1} min={0.1} /></div></div>}
              {(strategyType === 'mean_reversion') && <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">MA period</Label><Input type="number" value={maPeriod} onChange={e => setMaPeriod(Number(e.target.value))} min={2} /></div><div><Label className="text-xs">Deviation threshold (σ)</Label><Input type="number" value={deviationThreshold} onChange={e => setDeviationThreshold(Number(e.target.value))} step={0.5} min={0.5} /></div></div>}
              ...
              {(strategyType === 'grid') && <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Grid spacing (%)</Label><Input type="number" value={gridSpacingPct} onChange={e => setGridSpacingPct(Number(e.target.value))} step={0.5} min={0.5} /></div><div><Label className="text-xs">Grid levels</Label><Input type="number" value={gridLevels} onChange={e => setGridLevels(Number(e.target.value))} min={2} /></div></div>}
              ...
              <div className="grid grid-cols-2 gap-3"><div><Label className="text-xs">Max hold (hrs)</Label><Input type="number" value={maxHoldingHours} onChange={e => setMaxHoldingHours(Number(e.target.value))} min={1} /></div><div><Label className="text-xs">Transaction fee (%)</Label><Input type="number" value={feePct} onChange={e => setFeePct(Number(e.target.value))} min={0} max={1} step={0.01} /></div></div>
              <div className="flex items-center gap-2"><Switch checked={compound} onCheckedChange={setCompound} /><Label className="text-xs">Compound (reinvest profits)</Label></div>
              {days <= 90 && <div className="flex items-center gap-2 text-xs text-blue-500 bg-blue-500/10 rounded-md px-3 py-2"><Zap className="size-3.5" /><span>Hourly data (hourly) — bardziej precyzyjny backtest</span></div>}
              {days > 90 && <div className="flex items-center gap-2 text-xs text-amber-500 bg-amber-500/10 rounded-md px-3 py-2"><AlertTriangle className="size-3.5" /><span>Daily data — backtest mniej precyzyjny (brak danych godzinowych dla &gt;90 dni)</span></div>}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {result && (<>
            <div className="flex items-center gap-2"><Badge variant={result.results.data_granularity === 'hourly' ? 'default' : 'outline'} className="gap-1">{result.results.data_granularity === 'hourly' ? <><Zap className="size-3" /> Hourly data</> : <><Clock className="size-3" /> Daily data</>}</Badge>{result.results.data_granularity === 'daily' && <span className="text-xs text-amber-500">Results may be less precise</span>}</div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <MetricCard label="Total return (net)" value={`${result.results.total_return_pct >= 0 ? '+' : ''}${result.results.total_return_pct.toFixed(2)}%`} sub={`Final capital: $${result.results.final_capital.toFixed(2)}`} color={result.results.total_return_pct >= 0 ? 'text-emerald-500' : 'text-red-500'} icon={<TrendingUp className="size-4" />} />
              <MetricCard label="Win Rate" value={`${result.results.win_rate.toFixed(1)}%`} sub={`${result.results.winning_trades}W / ${result.results.losing_trades}L / ${result.results.breakeven_trades}BE of ${result.results.total_trades} trades`} color={result.results.win_rate >= 50 ? 'text-emerald-500' : 'text-red-500'} icon={<Target className="size-4" />} />
            ...
            <Card><CardHeader><CardTitle className="text-base">Equity curve</CardTitle><CardDescription>{compound ? 'With reinvested profits (compound)' : 'Without reinvestment'} — {COIN_OPTIONS.find(c => c.id === coinId)?.label || coinId}, {days} dni ({result.results.data_granularity === 'hourly' ? 'hourly data' : 'daily data'})</CardDescription></CardHeader>
              <CardContent><ChartContainer config={{ capital: { label: 'Capital', color: result.results.total_return_pct >= 0 ? '#10b981' : '#ef4444' } }} className="h-[300px] w-full"><AreaChart data={result.equity_curve}><defs><linearGradient id="capitalGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={result.results.total_return_pct >= 0 ? '#10b981' : '#ef4444'} stopOpacity={0.3} /><stop offset="95%" stopColor={result.results.total_return_pct >= 0 ? '#10b981' : '#ef4444'} stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" opacity={0.1} /><XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(v) => new Date(v).toLocaleDateString('pl-PL', { month: 'short', day: 'numeric' })} /><YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => '$' + v.toLocaleString()} /><ChartTooltip content={<ChartTooltipContent />} /><Area type="monotone" dataKey="capital" stroke={result.results.total_return_pct >= 0 ? '#10b981' : '#ef4444'} fill="url(#capitalGradient)" strokeWidth={2} /></AreaChart></ChartContainer></CardContent>
            </Card>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card><CardHeader><CardTitle className="text-base">Detailed statistics</CardTitle></CardHeader>
                <CardContent><div className="space-y-3 text-sm">
                  <StatRow label="Avg. net profit (win)" value={result.results.avg_profit_pct > 0 ? `+${result.results.avg_profit_pct.toFixed(2)}%` : '0%'} color="text-emerald-500" />
                  <StatRow label="Avg. net loss (loss)" value={`${result.results.avg_loss_pct.toFixed(2)}%`} color="text-red-500" />
                  <StatRow label="Avg. net profit/trade" value={`${result.results.avg_net_profit_pct >= 0 ? '+' : ''}${result.results.avg_net_profit_pct.toFixed(2)}%`} color={result.results.avg_net_profit_pct >= 0 ? 'text-emerald-500' : 'text-red-500'} />
                  ...
                  <StatRow label="Avg. holding time" value={`${result.results.avg_holding_hours.toFixed(1)}h`} color="" />
                  <StatRow label="Win streak" value={`${result.results.consecutive_wins} in a row`} color="text-emerald-500" />
                  <StatRow label="Loss streak" value={`${result.results.consecutive_losses} in a row`} color="text-red-500" />
                  ...
                  <StatRow label="Total fees" value={`$${result.results.total_fees.toFixed(2)}`} color="text-amber-500" />
                  <StatRow label="Total slippage" value={`$${(result.results.total_slippage ?? 0).toFixed(2)}`} color="text-amber-500" />
                  <StatRow label="Slippage / trade" value={`${(result.results.slippage_pct ?? 0.05).toFixed(2)}%`} color="text-amber-500" />
                  <StatRow label="Wick simulation" value={result.results.wick_simulation ? 'Enabled' : 'Disabled'} color={result.results.wick_simulation ? 'text-emerald-500' : 'text-muted-foreground'} />
                </div></CardContent>
              </Card>
              <Card className="border-amber-500/30 bg-amber-500/5"><CardHeader className="pb-2"><div className="flex items-center gap-2"><AlertTriangle className="size-5 text-amber-500" /><CardTitle className="text-sm">Model wykonania</CardTitle></div></CardHeader>
                <CardContent className="text-xs space-y-2"><div className="flex items-center gap-2"><Badge variant={result.results.data_granularity === 'hourly' ? 'default' : 'outline'} className="gap-1 text-[10px]">{result.results.data_granularity === 'hourly' ? 'Hourly' : 'Daily'}</Badge><span className="text-muted-foreground">{result.results.data_granularity === 'hourly' ? 'Buy/sell at 1H candle close' : 'Buy/sell at 1D candle close'}</span></div>
                  {result.results.wick_simulation && <p className="text-amber-600">Wick simulation active — SL/TP also checked inside candles (estimated low/high based on ATR). Results are more realistic than without wick sim, but may still differ from live trading.</p>}
                  {!result.results.wick_simulation && <p className="text-red-500">Wick simulation disabled — SL/TP checked ONLY at candle close. May skip intra-candle stop-losses, giving unrealistically optimistic results.</p>}
                  <p className="text-muted-foreground">Slippage: {(result.results.slippage_pct ?? 0.05).toFixed(2)}% — realistic market order execution price deviation. Total slippage cost: ${(result.results.total_slippage ?? 0).toFixed(2)}</p>
                </CardContent>
              </Card>
              <Card><CardHeader><CardTitle className="text-base">Exit reasons distribution</CardTitle></CardHeader>
            ...
            {result.trades.length > 0 && (<Card><CardHeader className="pb-2"><CardTitle className="text-base">History trades</CardTitle><CardDescription>{result.trades.length} trades w okresie {days} dni | Total fees: ${result.results.total_fees.toFixed(2)}</CardDescription></CardHeader>
              <CardContent><ScrollArea className="h-[200px]"><div className="space-y-1"><div className="grid grid-cols-7 gap-2 text-xs font-medium text-muted-foreground px-2 py-1"><span>Entry</span><span>Entry price</span><span>Exit</span><span>Exit price</span><span>Gross</span><span>Net</span><span>Reason</span></div>{result.trades.map((trade, i) => (<div key={i} className="grid grid-cols-7 gap-2 text-xs px-2 py-2 rounded hover:bg-muted/50 items-center"><span className="text-muted-foreground">{new Date(trade.entry_date).toLocaleDateString('pl-PL', { month: 'short', day: 'numeric' })}</span><span>${trade.entry_price.toFixed(4)}</span><span className="text-muted-foreground">{new Date(trade.exit_date).toLocaleDateString('pl-PL', { month: 'short', day: 'numeric' })}</span><span>${trade.exit_price.toFixed(4)}</span><span className={trade.profit_pct >= 0 ? 'text-emerald-500/70' : 'text-red-500/70'}>{trade.profit_pct >= 0 ? '+' : ''}{trade.profit_pct.toFixed(2)}%</span><span className={trade.net_profit_pct >= 0 ? 'text-emerald-500 font-medium' : 'text-red-500 font-medium'}>{trade.net_profit_pct >= 0 ? '+' : ''}{trade.net_profit_pct.toFixed(2)}%</span><span className={exitReasonColor(trade.exit_reason)}>{exitReasonLabel(trade.exit_reason)}</span></div>))}</div></ScrollArea></CardContent>
            </Card>)}
          </>)}
        </div>
      </div>
      <Card><CardHeader className="pb-2"><div className="flex items-center gap-2"><CalendarDays className="size-4 text-red-500" /><CardTitle className="text-sm">Drawdown Map — Drawdown Heat Map</CardTitle></div><CardDescription className="text-xs">Maximum drawdown from highs for top 15 coins in last 7 days</CardDescription></CardHeader><CardContent><DrawdownHeatMap /></CardContent></Card>
    </div>
  )
}
