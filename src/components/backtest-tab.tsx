...
function DrawdownHeatMap() {
  ...
  return (<>
    ...
    {heatMapTooltip && <div className="mt-2 text-xs text-muted-foreground bg-muted/50 rounded px-3 py-1.5"><span className="font-medium">{heatMapTooltip.coin}</span> — day {heatMapTooltip.day === 0 ? 'dziś' : `-${heatMapTooltip.day}d`}: <span className={heatMapTooltip.drawdown < -5 ? 'text-red-500 font-medium' : 'text-amber-500'}>{heatMapTooltip.drawdown.toFixed(2)}%</span></div>}
