// ─── Shared Types, Constants, and Helpers for Trading ──────────────────
...
export function sanitizeImageUrl(url: string | null | undefined): string {
  ...
  try {
    ...
    if (!ALLOWED_IMAGE_PROTOCOLS.includes(parsed.protocol)) return ''
    return parsed.href
  } catch {
    return ''
  }
...
// ─── Sparkline counter (shared) ────────────────────────────────────────────

export const sparklineCounterRef = { current: 0 }
...
export function getDefaultParamsForType(type: string): Partial<StrategyConfig> {
  switch (type) {
    ...
    case 'dip_buying':
    default:
      return { dip_threshold_1h: 0, dip_threshold_24h: -3, take_profit_pct: 5, stop_loss_pct: 1.5, max_holding_hours: 48, trailing_stop_pct: 1.5 }
  }
}
...
export const DEFAULT_STRATEGIES: StrategyConfig[] = [
  ...
  },
  {
    ...
    ma_period: 20, volume_threshold: 1.5,
    deviation_threshold: 2, lookback_periods: 20, breakout_confirm_bars: 2,
...
export interface CompoundProjection {
  ...
  monthlyData: { month: number; capital: number; wins: number; losses: number; monthlyReturnPct: number; baseline: number }[]
  scenarios: { label: string; capital: number }[]
}

export function calculateCompoundProjection(params: {
  capital: number
  ...
  const { capital: initialCapital, profitPerTrade, tradesPerMonth, winRate, lossPct, months } = params

  let currentCapital = initialCapital
  const monthlyData: CompoundProjection['monthlyData'] = []
  let totalWins = 0
  let totalTrades = 0
  ...
  for (let m = 1; m <= months; m++) {
    ...
    for (let t = 0; t < wins; t++) {
      currentCapital = currentCapital * (1 + profitPerTrade / 100)
    }
    for (let t = 0; t < losses; t++) {
      currentCapital = currentCapital * (1 - lossPct / 100)
    }

    totalWins += wins
    totalTrades += tradesPerMonth

    const monthlyReturnPct = ((currentCapital - startCapital) / startCapital) * 100
  ...
  const avgMonthlyReturnPct = totalReturnPct / months
  const expectedPctPerTrade = (profitPerTrade * (winRate / 100)) - (lossPct * ((100 - winRate) / 100))
  const avgTradePct = expectedPctPerTrade

  const scenarios = [40, 50, 60, 70, 80].map(wr => {
    let cap = initialCapital
    for (let m = 0; m < months; m++) {
      const w = Math.round(tradesPerMonth * (wr / 100))
      const l = tradesPerMonth - w
      for (let t = 0; t < w; t++) cap = cap * (1 + profitPerTrade / 100)
      for (let t = 0; t < l; t++) cap = cap * (1 - lossPct / 100)
    }
    return { label: `WR ${wr}%`, capital: Math.round(cap) }
  })

  return {
    finalCapital: Math.round(currentCapital * 100) / 100,
    totalReturnPct: Math.round(totalReturnPct * 100) / 100,
    avgMonthlyReturnPct: Math.round(avgMonthlyReturnPct * 100) / 100,
